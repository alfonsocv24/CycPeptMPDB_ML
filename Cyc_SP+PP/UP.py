...
from keras.layers import Dense, LSTM, Conv1D, Input, Dropout, Add, Concatenate, Lambda
...
class CyclicPeptide:
    ...
    monomers = pd.read_csv("CycPeptMPDB_Monomer_All_FixSMI.csv")
    amino_acids = monomers['Symbol'].values
    amino_acid_values = {aa: i+1 for i, aa in enumerate(amino_acids)}
    terminal_aa_dict = {'ac-' : ['T'], 'deca-' : ['T'], 'medl-' : ['T'],
                        'glyco-' : ['K'], 'Mono21-' : ['Orn'], 'Mono22-' : ['dLeu(3R-OH)'],
    ...
    def __init__(self):
        ...
        seqs = self.seq_data['Sequence'].to_numpy()
        seq_prop_col = self.seq_data.columns.to_list() # Get columns of DF to list
        seq_prop_col = seq_prop_col[3:] # Get names of properties columns
        self.seq_properties = self.seq_data[seq_prop_col].to_numpy() # Get properties columns to np array 
        self.seq_properties = StandardScaler().fit_transform(self.seq_properties) # Scale properties
