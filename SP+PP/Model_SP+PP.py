...
def model_builder(hp):
    ...
    Layers:
        ...
        3. LSTM
        4. Concatenation to join SP and PP
    ...
    input_Seq_prop = Input(shape=(208,))
    ...
    # CONCATENATE THE INPUT FROM THE PP WITH THE OUTPUT OF LSTM
    concatenation = Concatenate(axis=-1)([LSTM_layer, input_Seq_prop])
    ...
    model = tf.keras.Model(inputs=[input_feat, input_Seq_prop], outputs = output_layer)
    hp_learning_rate = hp.Choice('learning_rate', values = [0.01, 1e-3, 1e-4])
...
for train, test in kfold.split(X, y):
    ...
    file_length = f'{dataset}_SPPP_split_lengths.dat'
    file_acc = f'Accuracy_SPPP_{dataset}.dat'
    file_metric = f'Metrics_SPPP_{dataset}.dat'
    if args.Trained:
        ...
        tuner.search([TRAIN, TRAIN_seq_prop], TRAIN_target, epochs = 2000,
                     validation_data = ([VAL, VAL_seq_prop], VAL_target),
                     callbacks = [early_stopping], batch_size = 32)
        # Get best tuning model
        model1 = tuner.get_best_models(num_models=1)[0]
        acc1 = model1.evaluate([TEST, TEST_seq_prop], TEST_target, verbose = 0)[1]
        ...
        ##TRAIN the model
        history = model2.fit([TRAIN, TRAIN_seq_prop], TRAIN_target, epochs = 2000,
                  validation_data=([VAL, VAL_seq_prop], VAL_target), batch_size= 32, verbose = 1,
                  callbacks=[early_stopping])
        
        acc2 = model2.evaluate([TEST, TEST_seq_prop], TEST_target, verbose = 0)[1]
    ...
    #EVALUATION
    scores_train = model.evaluate([TRAIN, TRAIN_seq_prop], TRAIN_target, verbose = 0)
    scores = model.evaluate([TEST, TEST_seq_prop], TEST_target, verbose = 0)
    scores_val = model.evaluate([VAL, VAL_seq_prop], VAL_target, verbose = 0)
    acc_per_fold.append(scores[1] * 100)
    loss_per_fold.append(scores[0])
    data_2_plot = [208, fold_no, scores_val[1]*100, scores[1]*100,scores_train[1]*100]
    sep = '    '
    # METRICS
    y_pred = model.predict([TEST, TEST_seq_prop])
    new_y = []
    new_y = []
