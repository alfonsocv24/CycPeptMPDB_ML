...
for train, test in kfold.split(X, y):
    ...
    file_length = f'{dataset}_PPSMI_split_lengths.dat'
    file_acc = f'Accuracy_PPSMI_{dataset}.dat'
    file_metric = f'Metrics_PPSMI_{dataset}.dat'
    if args.Trained:
        ...
        acc1 = model1.evaluate([TEST_seq_prop, TEST_SMILE], TEST_target, verbose = 0)[1]
        ...
        ##TRAIN the model
        history = model2.fit([TRAIN_seq_prop, TRAIN_SMILE], TRAIN_target, epochs = 2000,
                  validation_data=([VAL_seq_prop, VAL_SMILE], VAL_target), batch_size= 32, verbose = 1,
                  callbacks=[early_stopping_train])
        acc2 = model2.evaluate([TEST_seq_prop, TEST_SMILE], TEST_target, verbose = 0)[1]
