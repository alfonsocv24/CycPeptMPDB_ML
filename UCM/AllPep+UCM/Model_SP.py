...
parser.add_argument('-ds', '--DATASET', help='Dataset name, AllPep or L67',
                    ...
                    choices=['AllPep'])
parser.add_argument('-n', '--NUM', help='Number of mutations added',
                    action='store', type=int, default = 1500000,
                    choices=['AllPep'])
...
for train, test in kfold.split(X, y):
    ...
    # We will create dictionary with mutations of sequence in training
    only_mut = {}
    ...
    # We will get now as many mutations as specified with -n in the input
    # Eliminate mutations that have more than one permeability value
    mut_seqs = []
    mut_targets = []
    for seq in only_mut.keys():
        if len(set(only_mut[seq])) == 1:
            #Creating a set will reduce the list to unique values. If there is
            # only one, that means that the sequence belongs to one class
            mut_seqs.append(seq) # Append seq to list
            mut_targets.append(only_mut[seq][0]) # Append target to list
    only_muts = dict(zip(mut_seqs, mut_targets)) # create dict with seqs as key
    # Create a df with sequences as index
    df_mut = pd.DataFrame.from_dict(only_muts, orient = 'index', columns = ['Permeable'])
    # get a sample to balance data
    df_mut_sample = df_mut.sample(abs(args.NUM), random_state = 42)
    muts = df_mut_sample.index.to_numpy()

    # Append sequences to TRAIN set
    TRAIN = np.append(TRAIN, muts, axis = 0)
    TRAIN_target = np.append(TRAIN_target, df_mut_sample['Permeable'].to_numpy(), axis = 0)
    TRAIN, _ = encoder.encode(sequences = TRAIN, length = 15, stop_signal = False)
    TRAIN_length = len(TRAIN)
    tun = np.random.choice(TRAIN.shape[0], 50000, replace = False)
    #Test
    TEST = X[test]
    ...
    if args.Trained:
        ...
        tuner.search(TRAIN[tun], TRAIN_target[tun], epochs = 2000, validation_data = (VAL, VAL_target),
